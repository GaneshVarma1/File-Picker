...
export function useFilePicker() {
  ...
  const loadFiles = useCallback(async () => {
    if (isSearching || !isClient) return;
  ...
  useEffect(() => {
    if (isClient && isSignedInToGoogle()) {
      loadFiles();
    }
  }, [isClient, loadFiles]);
    ...
    async (fileId: string) => {
      try {
        ...
        if (success) {
          updateFileIndexStatus(
            fileId,
            !files.find((f) => f.id === fileId)?.isIndexed
          );
  ...
  const handleFilter = useCallback(
    async (filters: {
      ...
      setIsLoading(true);
      try {
        const filteredFiles = await filterFiles(currentFolder, {
          ...filters,
          name: searchQuery,
        });
  ...
  const handleBulkRemove = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const { success, removed } = await removeMultipleFilesFromIndex(
        selectedFiles
      );
      if (success) {
  ...
  const sortedFiles = [...(isSearching ? searchResults : files)].sort(
    (a, b) => {
  ...
  return {
    ...
    isAllSelected: isSearching
      ? selectedFiles.length === searchResults.length &&
    ...
    // Connection status
    isConnected: isClient && isSignedInToGoogle(),
  };
}
